...
class Education {
    ...
    private static function shape( \WP_Post $post ): array {
        return [
            ...
            'slug'           => $post->post_name,
            'institution'    => esc_html( get_post_meta( $post->ID, '_hpcms_institution', true ) ),
            'degree'         => esc_html( get_post_meta( $post->ID, '_hpcms_degree', true ) ),
            'fieldOfStudy'   => esc_html( get_post_meta( $post->ID, '_hpcms_field_of_study', true ) ),
